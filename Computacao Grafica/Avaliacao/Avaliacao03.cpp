...
GLuint cam = 0;
GLfloat fAspect = 1;
GLdouble rotaciona_rodas = 0;
GLdouble acelera_rodas = 0;
...
void displayPersp(void)
{
	...
	glPushMatrix();
	{
		...
		glPushMatrix();
		{
			...
			glRotatef(rotaciona_rodas, 0, 1, 0);
			glRotatef(acelera_rodas, 0, 0, 1);
...
void init()
{
	...
	GLfloat difusa [4] = {0.5, 0.5, 0.5, 1.0};
...
void keyboard(unsigned char key, int x, int y) {
	...
	switch (key) {
		...
		case 90:
			rotaciona_rodas -= 5;
			rotaciona_rodas = (rotaciona_rodas < -25) ? -25 : rotaciona_rodas;
			break;
